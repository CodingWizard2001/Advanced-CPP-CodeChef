// You are given a set of 2D coordinates represented as pairs of integers (x, y). Complete the given C++ function that sorts these coordinates based on their distance from the origin (0, 0).
...
vector < pair < int, int >> sortCoordinates(const vector < pair < int, int >> & coordinates) {
    ...
    for (int i = 0; i < n; i++) {
        distances.push_back({
            distance_squared(coordinates[i].first, coordinates[i].second),
            i
        });
    }
